document.addEventListener('DOMContentLoaded', () => {
    ...
    els.form.addEventListener('submit', (e) => {
        ...
        if (!branch || !empName || !equipment || !file) {
            showModal('error', 'بيانات ناقصة , Incomplete data', 'يرجى إكمال جميع الحقول , Please complete all fields..');
            return;
        }

        showModal('loading', 'جاري الإرسال , Sending', 'يرجى الانتظار ، Please wait...');
        ...
        reader.onload = function() {
            ...
            .then(() => {
                ...
                showModal('success', 'تم الإرسال', 'تم الإرسال التقرير بنجاح.');
                els.form.reset();
                els.remove.click();
            })
            .catch(err => {
                showModal('error', 'خطأ', 'تعذر الإرسال: ' + err.message);
            });
        };
    });
});